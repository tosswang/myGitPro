...
public class Run1
{
	...
	static public class MyTask extends TimerTask
	{
		...
		public void run()
		{
			try
			{			
				/**队列中的任务是一个个执行的，只要一个延迟，后面会跟着一串全部延迟**/
				Thread.sleep(5000);
			}
			catch (Exception e)
			{
	...
	public static void main(String[] str)
	{
		try
		{
			...
			/**
			 * 1.TimerThread源码第497行，taskFired = (executionTime<=currentTime)
			 * 是通过当前时间与执行时间比较，相等为true时，就会调用task中的run
			 * 2.第三个参数间隔多久再次执行，这个也看源码，处理的很牛B，就是把任务再次排到
			 * 队列中的后面的位置去，让他执行。查看TaskQueue类的656行到668行的代码
			 * **/
